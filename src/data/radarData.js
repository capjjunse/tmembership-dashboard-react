// 제휴사 이슈 레이더 데이터 — AIInsight.jsx 섹션 4 + Overview.jsx 이슈 카드 공유
// membership: 'partner'=현재 제휴 | 'candidate'=잠재 후보 | 'watchlist'=관심
// direction:  'neg'=부정 | 'pos'=긍정 | 'neu'=중립
// strength:   'strong'=강(긴급) | 'mid'=중(주목) | 'low'=약(모니터링)
export const trendSignals = [
  {
    brand: '스타벅스',
    membership: 'partner',
    telcos: [
      { id: 'kt',  label: 'KT',   prog: '상시',  benefit: '[전 등급] 사이즈업' },
      { id: 'lgu', label: 'LGU+', prog: 'VIP콕', benefit: '[VVIP] 아메리카노 1잔 무료 / [VIP] 더블 사이즈업 무료' },
    ],
    strength: 'mid',
    sources: ['DataLab', '뉴스'],
    direction: 'neg',
    dlSpike: null,
    news7d: 25,
    negScore: 870,
    headline: [
      '여름 e-프리퀀시 취소 · 신뢰 회복 추진',
      '결제액 2주 반등 · 탱크데이 전 수준 미달',
      '탱크데이 후폭풍 · 6월 결제 최저 기록',
    ],
    impact: 'KT 상시·LGU+ VIP콕 운영 중 · SKT 미운영 · 탱크데이 여파 지속, e-프리퀀시 취소로 불매 진정 모색',
    links: [
      { label: '스타벅스 여름 e-프리퀀시 취소 (뉴스핌)', url: 'https://www.newspim.com/news/view/20260714001003' },
      { label: '결제액 2주 연속 반등 (파이낸셜뉴스)', url: 'https://www.fnnews.com/news/202607082040122681' },
    ],
  },
  {
    brand: '메가커피',
    membership: 'partner',
    telcos: [
      { id: 'kt', label: 'KT', prog: '달달초이스 7월', benefit: '[전 등급] 아이스아메리카노 1천원 할인 (2매)' },
    ],
    strength: 'low',
    sources: ['뉴스'],
    direction: 'neg',
    dlSpike: null,
    news7d: 20,
    negScore: 260,
    headline: [
      '점주 경영난 호소 · 사모펀드 구조 비판',
      '칠성사이다 동반 인상 · 업계 도미노',
      '매출 5.6배·배당 100% 재무 리스크',
    ],
    impact: 'KT 달달초이스 7월 운영 중 · SKT·LGU+ 미운영 · 가격 인상·재무 건전성 복합 리스크',
    links: [
      { label: '메가커피·칠성사이다 가격 인상 (MBC뉴스)', url: 'https://imnews.imbc.com/replay/2026/nwtoday/article/6832436_37012.html' },
      { label: '매출 5.6배·배당 100% 재무 리스크 (자본시장뉴스)', url: 'https://www.jabon.co.kr/news/articleView.html?idxno=3621' },
    ],
  },
  {
    brand: '피자헛',
    membership: 'partner',
    telcos: [
      { id: 'skt', label: 'SKT', prog: '상시', benefit: '[VIP] 30% 할인 · [G·S] 20% 할인' },
      { id: 'kt',  label: 'KT',  prog: '상시', benefit: '[전 등급] 15% 할인' },
      { id: 'lgu', label: 'LGU+', prog: '상시', benefit: '[전 등급] 15% 할인 (최대 3만원)' },
    ],
    strength: 'low',
    sources: ['뉴스'],
    direction: 'pos',
    dlSpike: null,
    news7d: 10,
    negScore: 80,
    headline: [
      'PH코리아 동일매장 매출 20%↑ 성장세',
      '수퍼슈프림 등 오리지널 메뉴 라인 부활',
      '78개 매장 간판 무상 지원 · 재건 추진',
    ],
    impact: 'SKT·KT·LGU+ 3사 상시 운영 중 · PH코리아 출범 후 매출 회복세, 재건 안착 중',
    links: [
      { label: 'PH코리아 공식 출범 (뉴스핌)', url: 'https://www.newspim.com/news/view/20260601001155' },
      { label: '피자헛 재건 초대 대표 김정은 취임 (뉴스핌)', url: 'https://www.newspim.com/news/view/20260527000117' },
    ],
  },
  {
    brand: '메가박스',
    membership: 'partner',
    telcos: [
      { id: 'skt', label: 'SKT', prog: '상시', benefit: '[전 등급] 최대 4,000원 할인 (11,000원 이상 예매 시)' },
      { id: 'kt',  label: 'KT',  prog: '상시', benefit: '[전 등급] 최대 6,000원 할인 (동반 4인)' },
    ],
    strength: 'mid',
    sources: ['뉴스'],
    direction: 'neg',
    dlSpike: null,
    news7d: 20,
    negScore: 870,
    headline: [
      '문체부 긴급 간담회 · 지원 방안 논의',
      '위탁상영관 이중위기 · 정산채권 분류',
      '영화인연대 정산채권 보호 촉구',
    ],
    impact: 'SKT·KT 상시 운영 중 · 회생절차 진행, 채권 신고 설명회 예정 · 계약 유지 여부 모니터링',
    links: [
      { label: '영화계 정산채권 보호 촉구 (파이낸셜뉴스)', url: 'https://www.fnnews.com/news/202607080818389170' },
      { label: '문체부 영화계 지원 논의 (이데일리)', url: 'https://edaily.co.kr/News/Read?mediaCodeNo=257&newsId=05628486645513208' },
      { label: '롯데시네마·메가박스 합병 최종 결렬 (이투데이)', url: 'https://www.etoday.co.kr/news/view/2599276' },
    ],
  },
  {
    brand: '60계치킨',
    membership: 'candidate',
    telcos: [],
    strength: 'low',
    sources: ['뉴스'],
    direction: 'neg',
    dlSpike: null,
    news7d: 15,
    negScore: 280,
    headline: [
      '닭근위 튀김 논란 · 7.5 사과문 게재',
      '60마리 새기름 콘셉트 신뢰도 타격',
      '치킨 3만원 시대 · 가격 부담 가중',
    ],
    impact: '3사 미제휴 · 닭근위 품질 논란으로 브랜드 이미지 타격, 제휴 타이밍 재검토 필요',
    links: [
      { label: "'닭근위 튀김' 논란 사과 (시애틀엔)", url: 'https://www.seattlen.com/focus/15519' },
    ],
  },
  {
    brand: '설빙',
    membership: 'watchlist',
    telcos: [],
    strength: 'mid',
    sources: ['뉴스'],
    direction: 'neg',
    dlSpike: null,
    news7d: 40,
    negScore: 280,
    headline: [
      '장갑 그대로 배수구 청소→빙수 제조 영상',
      'SNS·언론 급확산 · 사실관계 확인 중',
      '여름 성수기 위생 논란 · 이미지 타격',
    ],
    impact: '3사 미제휴 · 여름 성수기 위생 논란으로 브랜드 이미지 타격, Tday 재계약 타이밍 재검토 필요',
    links: [
      { label: '빙수 만들던 장갑으로 배수구 청소 (파이낸셜뉴스)', url: 'https://www.fnnews.com/news/202607200627262588' },
      { label: '배수구 만진 손 바로 빙수로 (한국일보)', url: 'https://www.hankookilbo.com/news/article/A2026072013380003031' },
    ],
  },
];
